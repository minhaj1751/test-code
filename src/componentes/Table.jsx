...
const Table = () => {
    const items = ["Name", "Status", "Date & Time", "Actions"];
    const [openUpdate, setOpenUpdate] = useState(false);
    const [openAdd, setOpenAdd] = useState(false);
    const [openFilter, setOpenFilter] = useState(false);
    const [popOpen, setPopOpen] = useState(null);
    const [popoverPosition, setPopoverPosition] = useState({ top: 0, left: 0 });
    const [rows, setRows] = useState(staticClassData); // Initialize with static data
        ...
        <div className="relative w-full">
            <div className={`p-5 ${popOpen ? "pb-16" : ""}`}>
                <div className="w-full px-5 relative bg-base-200">
                    <div className="w-full py-3 text-primary-content flex flex-col lg:flex-row items-center justify-between gap-2 lg:gap-5">
                        <h1 className="font-semibold text-3xl">Class Table</h1>
                        ...
                        <div className="flex flex-col lg:flex-row items-center justify-end gap-2">
                            ...
                            <div className="flex items-center justify-end gap-2">
                                ...
                                {/* ADD */}
                                <button onClick={() => setOpenAdd(true)} className="flex items-center gap-1 font-bold text-primary-content bg-primary hover:bg-primary-hover px-3 py-2 text-base rounded cursor-pointer whitespace-nowrap">
                                    <GoPlusCircle />
                                    Add
                                </button>
                            </div>
                        ...
                        <table className="border-separate border-spacing-0 rounded-md w-full">
                            <thead className="h-10">
                                <tr className="uppercase text-center bg-accent text-primary-content font-medium text-base">
                                    ...
                                    {col.status && <th className="border border-accent px-4 py-2 whitespace-nowrap">Status</th>}
                                    {col.datetime && <th className="border border-accent px-4 py-2 whitespace-nowrap">Date & Time</th>}
                                    {col.actions && <th className="border border-accent px-4 py-2 whitespace-nowrap">Actions</th>}
                                </tr>
                            </thead>
            ...
            <div className={`${openAdd ? "fixed top-0 right-0 max-w-xl w-full bg-base-200 h-screen overflow-y-scroll p-5 z-50" : "hidden"}`}>
                <div className="flex items-center justify-between border-b-2 border-accent pb-2">
                    <h1 className="text-base sm:text-lg font-semibold">Create Class</h1>
                ...
                <form className="mt-4 space-y-4" onSubmit={handleCreateSubmit}>
                    ...
                    <div className="flex items-center justify-end gap-5">
                        <button type="button" className="bg-secondary text-primary-content py-2 px-4 rounded-md  cursor-pointer " onClick={() => { reset(); setOpenAdd(false) }}>Cancel</button>
                        <button type="submit" className="bg-primary text-primary-content py-2 px-4 rounded-md  cursor-pointer ">Add</button>
                    </div>
                </form>
            </div>

            {/* FILTER SIDE PANEL */}
            <div className={`${openFilter ? "fixed top-0 right-0 max-w-xl w-full bg-base-200 h-screen overflow-y-scroll z-40" : "hidden"}`}>
                ...
                <form className="mt-4 px-5 space-y-2" onSubmit={handleFilterSubmit}>
                    ...
                    <div className="flex items-center justify-end gap-5">
                        <button type="button" className="bg-secondary text-primary-content py-2 px-4 rounded-md" onClick={() => setOpenFilter(false)}>Cancel</button>
                        <button type="submit" className="bg-primary text-primary-content py-2 px-4 rounded-md">Apply</button>
                    </div>
                </form>
            </div>
            ...
            {openUpdate && editingRow && (
                <div className="fixed top-0 right-0 max-w-xl w-full bg-base-200 h-screen overflow-y-scroll p-5 z-50">
                    <div className="flex items-center justify-between border-b-2 border-accent pb-2">
                        <h1 className="text-base sm:text-lg font-semibold">Update Class</h1>
                    ...
                    <form className="mt-4 space-y-4" onSubmit={handleUpdateSubmit}>
                        ...
                        <div className="flex items-center justify-end gap-5">
                            <button type="button" className="bg-secondary text-primary-content py-2 px-4 rounded-md cursor-pointer" onClick={() => { setOpenUpdate(false); setEditingRow(null); }}>Cancel</button>
                            <button type="submit" className="bg-primary text-primary-content py-2 px-4 rounded-md cursor-pointer">Update</button>
                        </div>
                    </form>
                </div>
            )}
